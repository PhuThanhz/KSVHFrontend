// src/layouts/footer.technician.tsx
import React from "react";
import { useNavigate, useLocation } from "react-router-dom";
import { PATHS } from "@/constants/paths";
import { HomeOutlined, CalendarOutlined, BellOutlined, UserOutlined } from "@ant-design/icons";
...
const FooterTechnician: React.FC = () => {
    ...
    const menuItems = [
        ...
        { key: PATHS.TECHNICIAN.NOTIFICATIONS, icon: <BellOutlined />, label: "Thông báo" },
        { key: PATHS.TECHNICIAN.PROFILE, icon: <UserOutlined />, label: "Trang cá nhân" },
    ];

    return (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-3 px-6 flex justify-around items-center z-50 shadow-lg h-24">            {menuItems.map((item) => {
            ...
            return (
                <Tooltip key={item.key} title={item.label} placement="top">
                    ...
                    >
                        ...
                        {isActive && <div className="w-1 h-1 bg-pink-500 rounded-full mt-1"></div>}
                    </div>
                </Tooltip>
            );
        })}
